...
PUBLIC int do_hole_map()
{
	...
	phys_clicks info_space, i;
	
	info_space = mm_in.m1_i1/(sizeof(phys_clicks)*2);	/* bo dane typu phys_clicks, a to pary adr-rozm, wiec mnozenie przez 2 */
	i = 0;
	
	for(hp = hole_head; i < info_space && hp != NIL_HOLE && hp->h_base < swap_base; hp = hp->h_next)
	{
		buffer[i] = hp->h_len;
		i++;
		buffer[i] = hp->h_base;
	...
	buffer[i] = 0;						/* koniec bufora 0, bo tak w instrukcji */
  ...
  else
  {
  	do 
  	{
		...
		while (hp != NIL_HOLE && hp->h_base < swap_base) 
		{
			...
			{
				worst = hp;		/* spointer do najwiekszej dziury */
				prev_worst = prev_ptr;	/* pointer do dziury przed najwieksza dziura */
			}
			prev_ptr = hp;
			hp = hp->h_next;
		}
